...
export default function DailyChallenge() {
  ...
  if (!user) return (
    <div className="min-h-screen max-w-3xl mx-auto px-4 py-8 text-center" data-testid="challenge-login-prompt">
      ...
      <h1 className="text-3xl font-black text-white uppercase" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>Connectez-vous</h1>
      <p className="text-gray-500 mt-2">Connectez-vous pour participer au défi quotidien</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen max-w-3xl mx-auto px-4 py-8" data-testid="daily-challenge-page">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-2">Devinez le résultat et gagnez un joueur gratuit !</p>
      </div>

      {!challenge?.active ? (
        <Card className="bg-[#121212] border-white/5 p-12 text-center" data-testid="no-challenge">
          <Clock className="w-10 h-10 text-gray-600 mx-auto mb-4" />
          <p className="text-gray-400">Aucun match disponible pour le défi</p>
          <p className="text-gray-600 text-xs mt-1">Revenez demain ou synchronisez les matchs</p>
        </Card>
      ) : (
        <Card className="bg-[#121212] border-white/5 overflow-hidden" data-testid="challenge-card">
          {/* Match Header */}
          <div className="bg-gradient-to-r from-[#FF0055]/5 via-transparent to-[#00F0FF]/5 p-6 border-b border-white/5">
            <p className="text-[10px] text-gray-500 uppercase tracking-widest text-center mb-4">Match du jour</p>
            <div className="flex items-center justify-center gap-8">
              <div className="text-center">
                ...
                <p className="text-sm font-bold text-white">{challenge.match?.home_team?.short || 'HOME'}</p>
                <p className="text-[10px] text-gray-500">{challenge.match?.home_team?.name}</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-bold text-white">{challenge.match?.away_team?.short || 'AWAY'}</p>
                <p className="text-[10px] text-gray-500">{challenge.match?.away_team?.name}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            {isActive && !result && (
              <div data-testid="prediction-options">
                <p className="text-sm text-gray-400 text-center mb-4">Qui va gagner ?</p>
            ...
            {isPredicted && !result && (
              <div className="text-center space-y-4" data-testid="prediction-submitted">
                ...
                <p className="text-xs text-gray-500">Le résultat sera disponible une fois le match terminé</p>
                <Button
                  onClick={checkResult}
                  disabled={submitting}
                  variant="outline"
                  className="border-white/10 text-gray-300 hover:bg-white/5 rounded-sm"
            ...
            {isCompleted && !result && (
              <div className="text-center" data-testid="challenge-completed">
                ...
                <p className="text-xs text-gray-500 mt-2">Revenez demain pour un nouveau défi</p>
              </div>
            )}
              ...
              <div className="text-center space-y-4" data-testid="challenge-result">
                {result.result === 'won' ? (
                  <>
                    ...
                    {result.player && (
                      <div className="inline-block p-4 rounded-xl bg-black/30 border border-white/10 mt-2">
                        ...
                        <p className="text-white font-bold text-sm mt-1">{result.player.name}</p>
                        <div className="flex items-center justify-center gap-1 mt-1">
                          <Star className="w-3 h-3 text-[#FFD700]" />
                          <span className="font-mono-data text-sm text-white">{result.player.rating}</span>
                        </div>
                      </div>
                    )}
                  </>
